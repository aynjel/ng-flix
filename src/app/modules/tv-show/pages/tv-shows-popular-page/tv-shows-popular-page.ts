...
@Component({
  selector: 'app-tv-shows-popular-page',
  imports: [DatePipe, UpperCasePipe, RouterLink],
  templateUrl: './tv-shows-popular-page.html',
})
export class TvShowsPopularPage implements OnInit, OnDestroy {
  private tvShowsService = inject(TvShowsService);

  protected popularTvShows: WritableSignal<TTvShow[]> = signal([]);
  ...
  ngOnInit(): void {
    this.tvShowsService
      ...
      .subscribe({
        next: (response) => this.popularTvShows.set(response.results),
        error: (errorResponse) => {
